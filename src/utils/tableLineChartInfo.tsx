...
export const ColusaTableLineChartInfo = ({ data,isDefault }: ChartParameters) => {
  ...
  const table_2025 = () => {
    return (
      <table className="dataframe text-white w-full border-separate border-spacing-x-3 -ml-2">
        ...
        <tbody>
          <tr>
            <td className="text-left">2025</td>
            <td className="text-left">Jan</td>
            <td className="text-right">206.2</td>
            <td className="text-right">100.0%</td>
            <td className="text-right">0.0</td>
          ...
          <tr>
            <td className="text-left">2025</td>
            <td className="text-left">Sep</td>
            <td className="text-right">33.5</td>
            <td className="text-right">16.8%</td>
            <td className="text-right">30</td>
            <td className="text-right">172.7</td>
          </tr>
        </tbody>
      </table>
    )
  ...
  if (isDefault) {
    return (
      <div className=' pl-4 pb-4 pt-2 w-full h-full overflow-y-auto overflow-x-hidden  flex flex-col gap-3 '>
        <div className='pl-2 flex justify-between items-center w-[92%] text-slate-800 dark:text-white gap-2'> <BasicSelect label='Year' Value={way} showLabel={false} setValue={(value) => setWay(value)} itemList={
          [{
            value: "2025",
            label: "2025"
          },
  ...
  return (
    <div className='p-1 h-full w-full flex gap-4'>
      ...
      <div className='flex w-3/5 h-full text-white'>
        <RTLineChart way={way} isColusa={true} />
      </div>
    </div>
  );
};
    ...
    <div className='flex-col w-[140px] '>
      ...
      <div className="w-full flex flex-row justify-between">
        <Button className="text-white bg-slate-900/90 h-6 px-2 text-xs" onClick={() => {
          window.open(`/msmtPoint-field?mpId=${mpId}&&wapId=${wapId}&&msmtpoint=${msmtPointId}`, '_blank');
        }}>
          View Detail
        </Button>
        <Button className="text-white bg-slate-900/90 h-6 px-2 text-xs" onClick={() => {
