...
const formSchema = z.object({
  ...
  fieldIrrigArea: z.coerce.number().optional(),
  fieldLegalArea: z.coerce.number().optional(),
    ...
    <FormPageWrapper>
      <FormPageHeader>
        <PageHeader
          ...
          breadcrumbPathList={[{ menuName: "Management", menuPath: "" }, { menuName: "Fields", menuPath: "/fields" }]}
        />
        {!viewMode && <Button className='w-24 mt-4' form="fieldForm" disabled={creatingField || updatingField} type="submit">{location.pathname.includes("edit") ? "Update" : "Add"}</Button>}
      </FormPageHeader>

      <FormWrapper>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} id="fieldForm" className='h-auto'>
            <FormFieldsWrapper>
              <FormComboBox control={form.control} name='wapId' label='Water Accounting Period' options={waps?.data} disabled={id ? true : false} />
              <FormInput control={form.control} name='fieldId' label='Field ID' placeholder='Enter Field ID' type='text' disabled={viewMode} />
              <FormInput control={form.control} name='fieldName' label='Field Name' placeholder='Enter Field Name' type='text' disabled={viewMode} />
